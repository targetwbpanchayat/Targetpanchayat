...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleOtpChange = (index: number, value: string) => {
    if (value.length > 1) {
      ...
      if (pasted) {
        ...
        setOtp(newOtp);
        document.getElementById(`modal-otp-input-${Math.min(5, pasted.length)}`)?.focus();
      }
      return;
    }
    const newOtp = [...otp];
  ...
  // 1. Registration — send OTP via Apps Script
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("ওটিপি পাঠাতে সমস্যা। পুনরায় চেষ্টা করুন।");
    } finally {
      setLoading(false);
    }
  };

  // 2. OTP Verification — verify via Apps Script, then save user locally
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    if (!user) return setError("এই ইমেইলে কোনো অ্যাকাউন্ট পাওয়া যায়নি। রেজিস্ট্রেশন করুন।");
    if (!user.isVerified) return setError("অ্যাকাউন্ট যাচাই করা হয়নি। রেজিস্ট্রেশন সম্পন্ন করুন।");
    if (user.passwordHash !== btoa(password)) return setError("ভুল পাসওয়ার্ড! সঠিক পাসওয়ার্ড দিন।");
  ...
  // 5. Forgot Password — send reset OTP via Apps Script
  const handleForgotPasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("পাসওয়ার্ড রিসেট OTP পাঠানো যায়নি।");
    } finally {
      setLoading(false);
    }
  };

  // 6. Reset Password Confirm — verify OTP, update password
  const handleResetPasswordConfirm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (idx !== -1) {
          users[idx].passwordHash = btoa(password);
        ...
        setError(res.message || "ভুল ওটিপি!");
      }
    } catch {
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-3.5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="youremail@gmail.com" className="w-full pl-10 pr-3 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none text-sm" required />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none text-sm" required />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                    {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
              </div>
              <button type="submit" disabled={loading} className="w-full py-2.5 rounded-xl bg-emerald-600 text-white font-semibold hover:bg-emerald-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer">
                {loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <>লগইন করুন <ArrowRight className="w-4 h-4" /></>}
              </button>
              <button type="button" onClick={() => { setMode("forgot"); setError(null); setSuccessMsg(null); }} className="w-full text-sm text-slate-500 hover:text-emerald-600 transition-colors">
              ...
              <button type="button" onClick={handleDemoLogin} className="w-full py-2.5 rounded-xl border border-slate-200 text-slate-700 font-medium hover:bg-slate-50 transition-colors flex items-center justify-center gap-2">
                <Sparkles className="w-4 h-4 text-amber-500" /> ডেমো হিসেবে দেখুন
              </button>
              <p className="text-center text-sm text-slate-500">
                অ্যাকাউন্ট নেই?{" "}
                <button type="button" onClick={() => { setMode("register"); setError(null); setSuccessMsg(null); }} className="text-emerald-600 font-semibold hover:underline">রেজিস্ট্রেশন করুন</button>
              </p>
            </form>
          )}
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-3">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="আপনার নাম" className="w-full pl-10 pr-3 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none text-sm" required />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">ইমেইল</label>
              ...
              <div>
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="ন্যূনতম ৬ অক্ষর" className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none text-sm" required />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                    {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <input type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="পাসওয়ার্ড পুনরায় লিখুন" className="w-full pl-10 pr-3 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none text-sm" required />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <select value={targetPost} onChange={(e) => setTargetPost(e.target.value)} className="w-full pl-10 pr-3 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none text-sm appearance-none bg-white">
              ...
              <p className="text-center text-sm text-slate-500">
                ...
                <button type="button" onClick={() => { setMode("login"); setError(null); setSuccessMsg(null); }} className="text-emerald-600 font-semibold hover:underline">লগইন করুন</button>
              </p>
            </form>
          )}
            ...
            <div className="space-y-4">
              <div className="text-center">
                ...
                <p className="text-sm text-slate-600">
                  আপনার ইমেইল <strong className="text-slate-900">{email}</strong> এ ৬-সংখ্যার ওটিপি পাঠানো হয়েছে। ইনবক্স বা স্প্যাম ফোল্ডার চেক করুন।
                </p>
              </div>

              <form onSubmit={mode === "otp" ? handleVerifyOtpSubmit : handleResetPasswordConfirm} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2 text-center">৬-সংখ্যার ওটিপি</label>
                  <div className="flex gap-1.5 justify-center">
                    {otp.map((digit, index) => (
                      <input key={index} id={`modal-otp-input-${index}`} type="text" inputMode="numeric" maxLength={1} value={digit} onChange={(e) => handleOtpChange(index, e.target.value)} onKeyDown={(e) => handleOtpKeyDown(index, e)} className="w-10 h-12 text-center text-lg font-bold rounded-lg border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none" />
                    ))}
                  </div>
                </div>
                ...
                {mode === "reset_otp" && (
                  <div>
                    ...
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                      <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="নতুন পাসওয়ার্ড" className="w-full pl-10 pr-3 py-2.5 rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none text-sm" required />
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between text-xs">
                  {canResend ? (
                    <button type="button" onClick={() => { setOtpTimer(60); setCanResend(false); mode === "otp" ? sendRegistrationOtp(email.trim(), name.trim()) : sendResetPasswordOtp(email.trim()); }} className="text-emerald-600 font-medium hover:underline">পুনরায় পাঠান</button>
                  ) : (
          ...
          {mode === "forgot" && (
            <form onSubmit={handleForgotPasswordSubmit} className="space-y-4">
              <div className="text-center">
                ...
                <p className="text-sm text-slate-600">আপনার নিবন্ধিত ইমেইল দিন। আমরা রিসেট OTP পাঠাব।</p>
              </div>
              <div>
              ...
              <button type="submit" disabled={loading} className="w-full py-2.5 rounded-xl bg-amber-600 text-white font-semibold hover:bg-amber-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer">
                {loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <>রিসেট OTP পাঠান <ArrowRight className="w-4 h-4" /></>}
              </button>
